"""
Тестовая версия главного файла с моковыми данными
Используется для тестирования без токена API
"""
import asyncio
import sys
...
from parsers.mock_parser import MockGoszakupParser  # Используем моковый парсер
...
class GoszakupMonitoringSystemTest:
    """Тестовая версия системы мониторинга с моковыми данными"""

    def __init__(self):
        self.bot = Bot(token=TELEGRAM_BOT_TOKEN)
        self.dp = get_dispatcher()
        self.parser = MockGoszakupParser()  # Моковый парсер вместо реального
    ...
    async def parse_and_notify(self):
        ...
        logger.info("🧪 ТЕСТОВЫЙ РЕЖИМ: Запуск парсинга с моковыми данными...")

        # Создать лог парсинга
        log = ParsingLogCRUD.create()

        try:
            # Парсинг лотов (используем моковые данные)
            ...
            logger.info(f"📊 Найдено тестовых лотов: {total_found}")
            ...
            logger.info(f"✅ Тестовый парсинг завершен. Новых: {new_added}, Дубликатов: {duplicates}")
    ...
    async def start_parsing_schedule(self):
        ...
        logger.info(f"⏰ Тестовый планировщик запущен. Интервал: 1 минута")

    async def start(self):
        """Запуск системы"""
        logger.info("=" * 60)
        logger.info("🧪 ТЕСТОВЫЙ РЕЖИМ: Запуск системы с моковыми данными")
        logger.info("=" * 60)
...
def main():
    """Точка входа"""
    try:
        system = GoszakupMonitoringSystemTest()
